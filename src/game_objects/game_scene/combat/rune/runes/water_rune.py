...
class WaterRune(Rune):
    ...
    def __init__(self, level: int, cost: int, max_energy_bonus: int, activation_time: int, energy_cost: float, activation_effect: RuneEffect, *args, **kwargs):
        super().__init__(self._sprite_file_path, f'Runa de Agua {level}', 'isso e uma runa de agua.', cost, max_energy_bonus, activation_time, energy_cost, activation_effect, *args, **kwargs)
